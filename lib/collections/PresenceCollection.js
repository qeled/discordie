...
function updatePresence(guildId, userId, status, game, client_status) {
  ...
  if (status === StatusTypes.OFFLINE) {
    ...
    delete this._client_statuses[userId]
    delete this._games[userId];
  } else {
    // set current presence
    if (client_status !== undefined) client_status = Object.assign({web: "offline", desktop: "offline", mobile: "offline"}, client_status)
    presencesForUser[guildId] = {status, game, client_status};

    // update global status and game (for user statuses in DM list)
    this._statuses[userId] = status;
    this._client_statuses[userId] = client_status;
...
class PresenceCollection {
  ...
  getClientStatus(userId, guildId) {
    if (this._discordie._user && this._discordie._user.id == userId) {
      return this._discordie._user.client_status;
    }
    const presence = getPresence.call(this,
      this._presencesForGuilds, userId, guildId
    ) || {};
    return presence.client_status || {web: "offline", desktop: "offline", mobile: "offline"};
  }
