...
const Constants = {
  ...
  Events: {
     ...
     *
     * Can return a raw websocket error `Number` or following `Error` messages:
     * - `` `Disconnected from gateway socket: ${error}` ``
     * - `"Failed to connect to voice: VOICE_SESSION_DESCRIPTION timed out"`
     * - `"Voice transport timed out"`
     * - `"Connected from another location"`
     * - `"Kicked from the voice channel"`
     ...
     * @property {VoiceSocket} socket
     * @property {IVoiceConnection} voiceConnection
     * @property {Error|Number|null} error
     * @property {boolean} manual -
     *                     indicating whether was caused by
     *                     IVoiceChannel.leave() or client.disconnect(),
     *                     also true if channel/guild has been deleted
     * @property {Promise} endpointAwait -
     *                     indicates whether there is a reconnect pending,
     *                     reconnects can occur when Discord decides to move
     *                     users to another voice server
     */
    ...
    /**
     * Note: Property `member` will contain `IUser` instance if user has left
     *       the `guild`.
     *
