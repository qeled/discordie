...
const Constants = {
  ...
  Events: {
     ...
     * - ** `"Login failed"` **
     *
     *   Set if `login` endpoint returns an error.
     *
     * - ** `"Could not get gateway"` **
     *
     *   Set if `gateway` endpoint returns an error.
     *
     * - ** `"No token specified"` **
     *
     ...
     *
     *   Set if server did not return a `READY` (initialization) packet after
     *   within 5 minutes.
     *
     ...
     *
     * All objects except offline members of large guilds (250+ members)
     * will be in cache when this event fires.
     *
