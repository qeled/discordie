...
class Discordie {
  constructor(options) {
    ...
    this._messageDeletePump = new RequestQueue();
    this._messageBulkDeletePump = new RequestQueue();
    this._patchGuildMembersPump = new RequestQueue();
    if (!this.options.messageQueue) {
      this._messagePump.disabled = true;
      this._messageDeletePump.disabled = true;
      this._messageBulkDeletePump.disabled = true;
      this._patchGuildMembersPump.disabled = true;
    }
     ...
     */
    this.VoiceConnections = new VoiceConnectionCollection(this, gw);

    /**
     * An array of unavailable guild's ids.
     * @returns {Array<String>}
     */
    this.UnavailableGuilds = new UnavailableGuildCollection(this, gw);
