...
function handleAuthLoginError(error) {
  this.pendingLogin = false;

  this.Dispatcher.emit(
    Events.DISCONNECTED,
    {error: new DiscordieError("Login failed", error)}
  );
}
function handleAuthLoginSuccess(e) {
  ...
  rest(this).gateway()
    .then(handleGatewaySuccess.bind(this))
    .catch(handleGatewayError.bind(this));
}

function handleGatewayError(error) {
  this.pendingLogin = false;

  var ev = {error: new DiscordieError("Could not get gateway", error)};
...
const defaultOptions = {
  ...
  messageQueue: true,
  largeThreshold: 50,
  intents: Object.keys(Constants.Intents).filter(i => i !== "GUILD_MEMBERS" && i !== "GUILD_PRESENCES" && i !== "ALL")
};
 ...
 *
 *   // Gateway large threshold
 *   // 'Number' between 50 and 250, total number of members where the
 *   // gateway will stop sending offline members in the guild member list
 *   largeThreshold: 50, // < default
 *
 *   // Gateway intents:
 *   // If present, an array of gateway intents or bitmask representing the intents
 *   // you wish the bot receive from Discord
 *   // otherwise all but privileged intents are passed on
 *   // Privileged intents are GUILD_MEMBERS and GUILD_PRESENCES
 *   // you may use ALL to receive all events
 *   intents: ["GUILDS", "GUILD_BANS", "GUILD_EMOJIS", "GUILD_INTEGRATIONS",
 ...
 *   "DIRECT_MESSAGE_REACTIONS", "DIRECT_MESSAGE_TYPING"] // < default
 *   intents: 32509 // < alternative method
 * });
 * ```
 */
class Discordie {
  ...
  connect(credentials) {
    ...
    rest(this).auth.login(credentials)
      .then(handleAuthLoginSuccess.bind(this))
      .catch(handleAuthLoginError.bind(this));
  }
  ...
  _createPrimaryGateway() {
    if (!this.gatewaySocket) {
      const gatewayOptions = {
        compressMessages: this.options.compressMessages,
...
Discordie.GuildExplicitContentFilterTypes = Constants.GuildExplicitContentFilterTypes;
...
Discordie.AuditLogAction = Constants.AuditLogAction;
Discordie.Intents = Constants.Intents;
