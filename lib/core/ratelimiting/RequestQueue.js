...
class RequestQueue {
  ...
  _drain() {
    ...
    entry.request.send((err, res) => {
      ...
      if (err && res && res.status === 429) {
        ...
        this._scheduleDrain(retryAfter, true);
        return;
      }
  ...
  _updateBucket(res) {
    ...
    if (belowPrecision) {
      this.bucket.rescheduleRefill(this.bucket.duration);
    }
    else if (res.headers["x-ratelimit-reset"] && res.headers["date"]) {
  ...
  _scheduleDrain(time, isRatelimited) {
    this.timeout = setTimeout(() => {
      this.timeout = null;
      this._drain();
    }, time + (isRatelimited ? POST_RATELIMIT_DELAY : 10));
  }
}
