...
class RequestQueueManager {
  constructor(discordie) {
    ...
    // whole API, 50/1s
    const _bot_global =
      this._createBucket(50, 1000, "bot:global");
    ...
    const _bot_global_generic =
      this._createBucketFactory(250, 5000, "bot:global:generic", _bot_global);
    this.genericRequestQueues = new RequestQueueGroup(_bot_global_generic);
  ...
  put(request, sendCallback) {
    const route = request.path.replace(/\/\d+$/g, "");
    // strip last url param: <- /api/guilds/:guild_id/bans/:user_id
    //                       -> /api/guilds/:guild_id/bans
