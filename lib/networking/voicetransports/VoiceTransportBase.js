...
const RTCP_TYPE_SENDER_REPORT = 0xC8;
const RTCP_TYPE_RECEIVER_REPORT = 0xC9;
const RTCP_TYPE_SOURCE_DESCRIPTION = 0xCA;
...
class VoiceTransportBase {
  constructor(voicews, ssrc) {
...

    {
      ...
      this.rtcpSenderReport[1] = RTCP_TYPE_SENDER_REPORT;
      ...
      this.rtcpSenderReport[29] = RTCP_TYPE_SOURCE_DESCRIPTION;
  ...
  demux(packet) {
    ...
    const extended = packet[0] & (1 << 4);
    const type = packet.readUInt8(1);
    ...
    if (extended && packet.length < 14)
      return;

    const validExtensionMagic =
      extended && packet.readUInt16BE(12) === 0xBEDE;

    var extensionLength = 0;

    if (validExtensionMagic && packet.length >= 16) {
      // be de : magic
      // 00 01 : numExtensions
      extensionLength += 4;

      // skip N 32 bit words
      const numExtensions = packet.readUInt16BE(14);
      extensionLength += numExtensions * 4;
    }

    if (packet.length < 12 + extensionLength)
      return;

    const chunk = packet.slice(12 + extensionLength);
