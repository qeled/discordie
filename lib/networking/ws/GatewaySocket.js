...
class GatewaySocket {
  ...
  voiceStateUpdate(guildId, channelId, selfMute, selfDeaf, external) {
    ...
    if (guildId && channelId && !voiceState) {
      this.voiceStates.set(guildId, voiceState = {});
    }

    if (guildId && !channelId && voiceState) {
      if (!external) this.disconnectVoice(guildId);
      this.voiceStates.delete(guildId);
    }

    if (guildId && !channelId && !voiceState) return;
  ...
  createVoiceSocket(endpoint, guildId, userId, sessionId, token) {
    ...
    const canReconnect = true; // endpoint null means server will send it later?
    this.disconnectVoice(
      guildId, canReconnect,
  ...
  disconnectVoice(guildId, reconnect, error) {
