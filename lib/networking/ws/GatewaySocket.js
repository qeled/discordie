...
class GatewaySocket {
  ...
  connect(url) {
    ...
    const close = (error) => {
      ...
      if (this.isPrimary) {
        this.Dispatcher.emit(
          Events.DISCONNECTED,
          {error: new DiscordieError("Disconnected from primary gateway", error)}
        );
      }
    };
  ...
  disconnect(error, causedByEvent) {
    if (!causedByEvent) {
      this.seq = 0;
  ...
  disconnectVoice(reconnect) {
    if (this.voiceSocket) {
      if (!reconnect && this.connected)
        this.voiceStateUpdate(null, null);

      this.voiceSocket.disconnect(null, true);
      this.voiceSocket = null;
    }
  }
