...
class VoiceSocket {
  constructor(_gateway) {
    ...
    this.audioScheduler = this.audioEncoder; // todo: remove, legacy
  ...
  connect(server, serverId, userId, sessionId, voiceToken) {
    ...
    this.socket.on("message", e => {
      ...
      if (op === OPCODE_SESSION_DESCRIPTION) {
        ...
        // 960 samples of silence, opus encoded
        // required to start receiving audio from other users
        // (can be really any audio)
        this.audioTransportSocket.send(new Buffer([0xF8, 0xFF, 0xFE]), 960);
      }
  ...
  disconnect(error, description, causedByEvent) {
    ...
    if (closedTransport || causedByEvent) {
      let msg = {socket: this};
    ...
    if (this.audioEncoder) {
      ...
      this.audioScheduler = null; // todo: remove
    }
  ...
  connectAudioTransport(ssrc, serverPort, modes) {
    ...
    transport.onTimeout = () =>
      transport.onError(new DiscordieError("Voice transport timed out"));
