...
function processAudio() {
  ...
  const channel = new ArrayBuffer(bufferLength);
  ...
  encoded = encode(view).map(packet => Utils.createBuffer(packet));
  DiscordieProfiler.stop("encode");

  if (!encoded.length)
    console.log("check your audio payload, buffer size smaller than expected");
