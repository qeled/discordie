...
class IVoiceChannel extends IChannel {
  ...
  leave() {
    const v = this._discordie.VoiceConnections.getForChannel(this.channelId);
    if (v && v.gatewaySocket) v.gatewaySocket.disconnect();
  }
  getVoiceConnection() {
    return this._discordie.VoiceConnections.getForChannel(this.channelId);
  }
}
