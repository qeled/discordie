...
class IGuild extends IBase {
  ...
  ban(user, deleteMessageForDays, reason) {
    user = user.valueOf();
    console.log(reason);
    console.log(deleteMessageForDays)
  ...
  unban(user, reason) {
    user = user.valueOf();
    return rest(this._discordie).guilds.bans.unbanMember(this.id, user, reason);
  }
  ...
  /**
   * @param {IUser|String|null} [user_id] - User or user id
   * @param {String|null} [action_type] - Action type
   * @param {String|null} [before] - Audit event id
   * @param {Number|null} [limit] - Default is 50
   * @returns {Promise<Object, Error>}
   * @example
   */

  getAuditLogs(user_id, action_type, before, limit) {
    if (user_id) user_id = user_id.valueOf();
    if (before) before = before.valueOf();
    if (limit === undefined) limit = 50;
    if (limit <= 1) limit = 1;
    if (action_type) action_type = Constants.ActionTypes[action_type];

    return rest(this._discordie).guilds.getAuditLogs(this.id, user_id, action_type, before, limit);
  }
}
