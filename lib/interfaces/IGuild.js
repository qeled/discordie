...
class IGuild extends IBase {
  ...
  /**
   * Returns whether the guild requires MFA.
   * @returns {Boolean}
   * @readonly
   */
  get mfaRequired() {
  ...
  get categoryChannels() {
    return this._discordie.Channels.categoriesForGuild(this.id);
  }

  /**
   * Returns general channel of this guild.
   * this may return null as general channels have been deprecated for newer guilds.
   ...
   */
  createChannel(type, name, permissionOverwrites, bitrate, userLimit, parent) {
    ...
    return new Promise((rs, rj) => {
      rest(this._discordie)
        .channels.createChannel(this.id,
          type, name, permissionOverwrites, bitrate, userLimit, parent
        )
   ...
   * Permission bitfield, set to null for default permissions, 0 for none
   * @param {boolean} [hoist] - true if role should be displayed separately
   * @param {boolean} [mentionable]
   * @returns {Promise<IRole, Error>}
   * @example
   * const permissions =
   *   Discordie.Permissions.Text.READ_MESSAGES |
   *   Discordie.Permissions.Text.READ_MESSAGE_HISTORY;
   *
   * guild.createRole("A Very Creative Role Name", 0xFF3030, permissions);
   */
  createRole(name, color, permissions, hoist, mentionable) {
    return new Promise((rs, rj) => {
   ...
   *                               (min 0, max 7)
   * @param {String|null} reason - Message to display in the audit log entry for this event
   * @returns {Promise}
   */
  ban(user, deleteMessageForDays, reason) {
    user = user.valueOf();
    return rest(this._discordie)
      .guilds.bans.banMember(this.id, user, deleteMessageForDays, reason);
  }
  ...
  /**
   * Makes a request to fetch audit logs of this guild.
   * All parameters are optional.
   * @param {IUser|String|null} [userId] - IUser, guild member, or user id.
   * @param {Number|String|null} [auditLogAction] - Number or string representing the event to filter for
   * See Discordie.AuditLogAction
   * @param {String|null} [before] - Snowflake of an audit log event.
   * @param {Number|null} [limit] How many audit log entries to fetch.
   * @returns {Promise<Object, Error>}
   *
   */
  getAuditLogs (userId, auditLogAction, before, limit) {
    if (limit && (limit < 1 || limit > 100)) return Promise.reject(`Limit must be a number between 1 and 100, or null`)
    if (userId) userId = userId.valueOf()
    if (before) before = before.valueOf()
    if (typeof auditLogAction === 'string' && !Constants.AuditLogAction[auditLogAction]) {
      return Promise.reject('actionType is not valid.')
    } else if (typeof auditLogAction === 'number' && !Object.values(Constants.AuditLogAction).includes(auditLogAction)) {
      return Promise.reject('actionType is not valid.')
    } else {
      auditLogAction = typeof auditLogAction === "string" ? Constants.AuditLogAction[auditLogAction] : auditLogAction
    }
    return rest(this._discordie)
      .guilds.getAuditLogs(this.id, userId, auditLogAction, limit, before)
  }
