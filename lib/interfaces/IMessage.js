...
class IMessage extends IBase {
  ...
  /**
   * Makes a request to remove a reaction from this message with the specified
   * emoji.
   * @param {Object|String} emoji
   * Partial emoji `{id: String|null, name: String}` or a unicode emoji
  ...
  /**
   * Makes a request to delete all the reactions for a given emoji on a message.
   * @param {Object|String} emoji
   * Partial emoji `{id: String|null, name: String}` or a unicode emoji
   * @return {Promise}
   */
  removeReactionEmoji(emoji) {
    ...
    return rest(this._discordie)
      .channels.removeReactionEmoji(this.channel_id, this.id, emoji);
  }
