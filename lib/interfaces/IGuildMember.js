...
class IGuildMember extends IUser {
   ...
   */
  get clientStatus() {
    return this._discordie._presences.getClientStatus(this.id, this._guildId);
  }

  get previousClientStatus() {
    return this._discordie._presences.getPreviousClientStatus(this.id, this._guildId);
  }
   ...
   * @returns {Promise}
   */
  ban(deleteMessageForDays, reason) {
    return rest(this._discordie)
      .guilds.bans.banMember(
        this._guildId, this._userId, deleteMessageForDays, reason
      );
  }
