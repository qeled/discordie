...
class ITextChannel extends IChannel {
   ...
   * @param {IMessage|String|null} [around] - Message or message id
  ...
  fetchMessages(limit, before, after, around) {
    ...
    if (around) around = around.valueOf();
    return new Promise((rs, rj) => {
      rest(this._discordie).channels.getMessages(this.id, limit, before, after, around)
      .then(e => {
  ...
  /**
   * Makes a request to fetch a single message for this channel.
   *
   * Works only with bot accounts.
   * @param {IMessage|string} [message] - Message or an id string
   * @returns {Promise<IMessage, Error>}
   */
  fetchMessage(message) {
    message = message.valueOf();
    return new Promise((rs, rj) => {
      rest(this._discordie).channels.getMessage(this.id, message, true)
      .then(msg => {
        return rs(this._discordie.Messages.get(msg.id));
      })
      .catch(rj);
    });
  }
   ...
   * @param {IBase|Array<IBase>} [mentions] - (kept for compatibility)
   * @param {boolean} [tts]
   * @param {Object} [embed]
   * Refer to [official API documentation](https://discordapp.com/developers/docs/resources/channel#embed-object)
   * for embed structure description.
   ...
   * channel.uploadFile("test.png", "test.png", "file with attachment in an embed",
   * false, false, {image: {url: 'attachment://test.png'}});
   */
  uploadFile(readableStream, filename, content, mentions, tts, embed) {
    if (mentions === true) {
      tts = mentions;
      mentions = [];
    }
