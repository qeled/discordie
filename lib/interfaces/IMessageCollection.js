...
class IMessageCollection extends ICollectionBase {
  ...
  /**
   * Makes a request to fetch a message by id for the specified channel.
   *
   * Works only with bot accounts.
   *
   * This method does not add the message to cache.
   *
   ...
   *
   * Returns a promise that resolves to a JSON object of the message.
   *
  ...
  fetchMessage(messageId, channelId) {
    ...
    return rest(this._discordie)
      .channels.getMessage(message.channel_id, message.id, false);
  }
