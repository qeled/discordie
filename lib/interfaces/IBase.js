...
class IBase {
  ...
  get _valid() {
    try {
      var getters = this.__proto__._gettersByProperty;
      for (var classname in getters) {
        if (getters[classname].hasOwnProperty("id"))
          return getters[classname]["id"].call(this, "id");
      }
    } catch (e) {
      return false;
    }
